    ...
    <div class="table-report mt-3">
        <div class="table-head">
            <div class="table-tr">
                ...
                <div
                    data-column-key="conversion_visits_to_leads:default"
                    class="table-th text-th"
                    v-for="(titleItem, index) in reportTableColumns"
                    :key="index"
                >
                    <div class="container-th">
                        <div class="infoCell-1">
                            <div
                                class="title-1"
                                v-if="reportTableTitle && titleItem"
                            >
                                {{
                                    reportTableTitle[
                                        titleItem as keyof AnaliticColumnData
                                    ]
                                }}
                            </div>
                            <i
                                name="question-circle-o"
                                class="fa fa-question-circle-o icon-question undefined"
                            ></i>
                        </div>
                    </div>
                </div>
        ...
        <div class="table-body table-avg-sum">
            <div class="table-tr table-body-header">
                <div
                    class="table-td-first table-td border-right-0 fixed-column"
                >
                    <div class="dimension-1">
                        <div class="averageText-1499811922">Итого/Среднее</div>
                    </div>
                    ...
                    <div class="average">
                        <div>
                            {{
                                countVal({
                                    col: titleItem,
                                    val: reportTableTotal[
                                        titleItem as keyof AnaliticColumnData
                                    ],
                                })
                            }}
                            <b v-if="reportTableUnits">
                                {{ metricUnit(reportTableUnits[index]) }}
                            </b>
                        </div>
                    </div>
                </div>
        ...
        <div class="table-body">
            <div>
                <div
                    v-for="(row, index) in reportTableRows"
                ...
                >
                    <div class="body-row table-tr">
                        ...
                        >
                            <div class="dimension-1">
                                ...
                                >
                                    <span
                                        v-if="
                                            !isOpenChild[row.info.hash_position]
                                        "
                                        >+</span
                                    >
                                    <span
                        ...
                        <div
                            class="table-td"
                            v-for="(tdItem, index) in reportTableColumns"
                            :key="index"
                        >
                            <div>
                                <div
                                    :class="getClass( tdItem as keyof AnaliticColumnData,countVal({
                                            col: tdItem,
                                            val: row.column_data[
                    ...
                    <div class="table-parent-tr"></div>
                    <!-- children 1st -->
                    <div class="children" v-if="row.children.length > 0">
                        <childRowReport
                            :item-row-children="row.children"
                            :item-row-table-columns="reportTableColumns"
                            :item-report-table-units="reportTableUnits"
                        ></childRowReport>
                    </div>
                    <!-- end children 1st -->
                </div>
...
import childRowReport from "./child-row-report.vue";
...
const reportTableTitle = computed(() => reportWithChild.value?.columns_header);
const reportTableColumns = computed(() => reportWithChild.value?.columns);
const reportTableRows = computed(() => reportWithChild.value?.rows);
// const reportTableUnits = computed(() => reportWithChild.value?.columns_type);
const isOpenChild = ref<{ [key: string]: boolean }>({});
const isAddChild = ref<{ [key: string]: boolean }>({});

const reportTableUnits = computed(() => {
    const arrUnits: any = reportWithChild.value?.columns_type;
    let arr: any = [];
    if (reportTableColumns.value && arrUnits) {
        reportTableColumns.value?.forEach((item: any) => {
            arr.push(arrUnits[item]);
        });
    }
    return arr;
});
...
const maxElement = computed(() => {
    ...
    const obj = {
        ...
        count_leads_canceled: [],
        count_leads_in_progress: [],
        count_leads_paid: [],
        count_leads: [],
        visits: [],
        profit: [],
    ...
    if (reportWithChild?.value?.rows) {
        reportWithChild.value?.rows.forEach((row: AnaliticRowsItem) => {
            for (const key in row.column_data) {
                const columnKey = key as keyof AnaliticColumnData;

                if (row.column_data[columnKey] != undefined) {
                    obj[columnKey].push(row.column_data[columnKey]);
                } else {
                    obj[columnKey].push(0);
                }
            }
        });
        console.log("obj", obj);
...
const reportTableTotal = computed(() => {
    const obj = {
        ...
        price_goal: 0,
        cpc: 0,
        avg_cost_per_thousand_impressions: 0,
        count_leads_canceled: 0,
        count_leads_in_progress: 0,
        count_leads_paid: 0,
        count_leads: 0,
        visits: 0,
        profit: 0,
    ...
    if (reportWithChild?.value?.rows) {
        reportWithChild?.value?.rows.forEach((row: AnaliticRowsItem) => {
            for (const key in row.column_data) {
                if (
                    Object.prototype.hasOwnProperty.call(row.column_data, key)
                ) {
                    const columnKey = key as keyof AnaliticColumnData;

                    if (row.column_data[columnKey] != undefined) {
                        obj[columnKey] += row.column_data[columnKey];
                    } else {
                        obj[key as keyof AnaliticColumnData] += 0;
                    }
                }
            }
        });
...
const columnsTypes = () => {
    let arr: any = [];
    if (reportTableColumns.value && reportTableUnits.value) {
        reportTableColumns.value.forEach((item: string) => {
            arr.push(reportTableUnits.value[item]);
        });
    }
};

const getClass = (element1: any, element2: any) => {
    // console.log(element1, "==", element2);
    if (maxElement.value != undefined) {
        if (Math.max(...maxElement.value[element1]) == element2) {
            return "td-max";
        }
        if (Math.min(...maxElement.value[element1]) == element2) {
            return "td-min";
        }
    }
};
...
.table-td .td-max {
    border-bottom: 1px solid #119a22;
}
