...
export function getYtVideoId(url: string): string {
    const regExp =
        /^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|&v=)([^#&?]*).*/; //DEVELOP
...
export async function save_images_from_base64(canvas: HTMLCanvasElement) {
    let blob = (await new Promise((resolve) => canvas.toBlob(resolve))) as any;
    blob = new Blob([blob], { type: "image/jpeg" });
    const formData = new FormData();
    formData.append("image", blob, "blob_image.jpg");
    return Api.Image().POST(formData, false);
}
export function addSourceToVideo(
