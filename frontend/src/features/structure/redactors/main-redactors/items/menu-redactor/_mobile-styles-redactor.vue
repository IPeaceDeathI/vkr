<template>
    <template v-if="$store.getters['environment/isMobileViewport']">
        <v-row>
            <v-col>
                <v-switch
                    v-model="redefine"
        ...
        <v-row
            v-if="!parentIsBlockHeader"
            :class="{ 'disabled-dom': !redefine }"
        >
            <v-col>
        ...
        <v-row :class="{ 'disabled-dom': !redefine }">
            <v-col>
                <btn-text-redactor
...
import { Color, MenuMobileStyle } from "@/types";
...
type Props = {
    modelValue: MenuMobileStyle;
    parentIsBlockHeader: boolean;
};

type Emits = {
    "update:modelValue": [value: MenuMobileStyle];
    "change:modelValue": [value: MenuMobileStyle];
};
...
//using params
const redefine = computed<boolean>({
    get() {
        return modelValue.value.redefine;
    },
    set(value: boolean) {
        emit("change:modelValue", {
            ...modelValue.value,
